...
static void SetupVulkanWindow(ImGui_ImplVulkanH_Window* wd, VkSurfaceKHR surface, int width, int height)
{
  ...
  ImGui_ImplVulkanH_CreateOrResizeWindow(g_Instance, g_PhysicalDevice, g_Device, wd, g_QueueFamily, g_Allocator, width,
...
int main(int, char**)
{
  ...
  while (!done)
  {
    ...
    while (SDL_PollEvent(&event))
    {
      ...
      {
        // Note: your own application may rely on SDL_WINDOWEVENT_MINIMIZED/SDL_WINDOWEVENT_RESTORED to skip updating
        // all-together. Here ImGui_ImplSDL2_NewFrame() will set io.DisplaySize to zero which will disable rendering but
        // let application run. Please note that you can't Present into a minimized window.
    ...
    if (g_SwapChainRebuild && g_SwapChainResizeWidth > 0 && g_SwapChainResizeHeight > 0)
    {
      ...
      ImGui_ImplVulkanH_CreateOrResizeWindow(g_Instance, g_PhysicalDevice, g_Device, &g_MainWindowData, g_QueueFamily,
                                             g_Allocator, g_SwapChainResizeWidth, g_SwapChainResizeHeight,
                                             g_MinImageCount);
